...
class PayController extends Controller
{
    ...
    public function getWechatOrder(Request $request, ResponseFactory $response, NativePayOrder $order)
    {
        ...
        //file_get_contents("./../test.txt",json_encode($wechatOrder).PHP_EOL,FILE_APPEND);
        ...
        //file_get_contents("test.txt",json_encode($request->getData()).PHP_EOL,FILE_APPEND);
        \Log::debug(json_encode($request->getData()));
        return $response->json(['message' => '创建微信订单失败-充值'], 422);
        //return $response->json(['message' => json_encode($request->getData())], 422);
    }
